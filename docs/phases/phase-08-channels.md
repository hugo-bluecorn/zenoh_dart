...
### `bin/z_queryable_with_channels.dart`

Mirrors `extern/zenoh-c/examples/z_queryable_with_channels.c`:

```
Usage: dart run bin/z_queryable_with_channels.dart [OPTIONS]
...
### `bin/z_non_blocking_get.dart`

Mirrors `extern/zenoh-c/examples/z_non_blocking_get.c`:

```
Usage: dart run bin/z_non_blocking_get.dart [OPTIONS]
...
2. **Integration test**: `z_queryable_with_channels.dart` + `z_get.dart` — channel queryable works
3. **Integration test**: `z_queryable.dart` + `z_non_blocking_get.dart` — non-blocking polling works
