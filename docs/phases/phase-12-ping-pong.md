...
```
Usage: dart run bin/z_pong.dart
```
...
```
Usage: dart run bin/z_ping.dart [OPTIONS]
...
2. **Integration test**: Run `bin/z_pong.dart` + `bin/z_ping.dart` — ping prints latency results
